...
#here i am adding the comments for lab1 from lab2
...
Ya=np.sin(X) #sin(x) function where X is the vector above 
Yb=np.cos(X)
...
x3=np.arange(1,len(w)+1,1) #creating a vector of length 1, to len(w)
...
def driver():
    n = 100
    x = np.linspace(0,np.pi,n) #a vector with evenly spaced n points from 0 to pi
...
def Matrix_Calc(C,A):
    ...
    for row in C: #going through the rows
        for j in range(len(A[0])): #going through the coluns
            B[i][j]=dotProduct(A[:,j],row,len(A[:,j])) #doing the dot product for each index
        i+=1  
                       
    return B

s2 = np.array([[1, 2]]) #2d vector
P=np.array([[1,0],[2,3]]) #matrix where each index is the row
...
C=np.array([[1,1,1],[4,5,6],[7,8,9]]) #3x3 matrix 
w=np.array([[4,5,6]])
